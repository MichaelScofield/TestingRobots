...
start(RobotId, AccountId, MessageDealer) ->
  ...
  timing(moving, RobotId, MessageDealer, 30000, RobotMovingState). % 500 and 150 are the initial coordinates of a robot

timing(moving, RobotId, MessageDealer, Timeout, {X, Y, AccountId}) ->
  ...
  receive
    stop ->
      ...
      stop
  after Timeout ->
    timing(moving, RobotId, MessageDealer, Timeout, {NewX, NewY, AccountId})
  end.